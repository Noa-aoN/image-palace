...
RSpec.describe "Api::V1::Collections", type: :request do
  ...
  describe "POST /api/v1/boxes" do
    it "creates a collection" do
      expect {
        post "/api/v1/boxes", params: { collection: { name: "新コレクション" } }, headers: headers, as: :json
    ...
    it "returns validation error when name is blank" do
      post "/api/v1/boxes", params: { collection: { name: "" } }, headers: headers, as: :json
  ...
  describe "GET /api/v1/boxes/:id" do
    it "returns mixed entries (card / space / view)" do
      ...
      get "/api/v1/boxes/#{collection.id}", headers: headers, as: :json
    ...
    it "rejects access to another users collection" do
      ...
      get "/api/v1/boxes/#{other_collection.id}", headers: headers, as: :json
  ...
  describe "コレクションへのエントリ追加・削除" do
    it "adds a deck entry" do
      ...
      expect {
        post "/api/v1/boxes/#{collection.id}/entries",
    ...
    it "removes an entry" do
      ...
      expect {
        delete "/api/v1/boxes/#{collection.id}/entries/View/#{deck.id}", headers: headers, as: :json
      }.to change { collection.collection_entries.count }.by(-1)
  ...
  describe "DELETE /api/v1/boxes/:id" do
    it "deletes the collection" do
      ...
      expect {
        delete "/api/v1/boxes/#{collection.id}", headers: headers, as: :json
      }.to change { user.collections.count }.by(-1)
