...
RSpec.describe "Rack::Attack throttling for high-cost endpoints", type: :request do
  ...
  describe "意味生成のスロットル（OpenAI コスト）" do
    ...
    it "10 回を超えると 429 を返す" do
      freeze_time { 11.times { post "/api/v1/items/#{item.id}/meaning", headers: headers } }
      expect(response).to have_http_status(:too_many_requests)
    end

    it "上限内はスロットルしない" do
      freeze_time { 10.times { post "/api/v1/items/#{item.id}/meaning", headers: headers } }
  ...
  describe "タグ生成のスロットル（OpenAI コスト）" do
    ...
    it "10 回を超えると 429 を返す" do
      freeze_time { 11.times { post "/api/v1/items/#{item.id}/tags", headers: headers } }
      expect(response).to have_http_status(:too_many_requests)
    end

    it "上限内はスロットルしない" do
      freeze_time { 10.times { post "/api/v1/items/#{item.id}/tags", headers: headers } }
      expect(response).not_to have_http_status(:too_many_requests)
    end
  end
