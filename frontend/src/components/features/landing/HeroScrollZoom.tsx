  ...
  return (
    <section ref={trackRef} className="hero-track" data-reduced={reduced ? 'true' : 'false'}>
      <div
        ...
        // **狭い画面では、まん中に置く。**
        //
        // 上端から 7vh・11vh と下ろしてみたが、文字束（実測で画面の約4割）の
        // 中心が 32% あたりにしかならず、まだ上に寄って見えた。
        // 決め打ちの余白で「中央あたり」を狙うのをやめ、**余った縦を等分**する。
        // 端末の高さと文字の折り返しで束の高さは変わるので、数値で当てにいかない。
        //
        // 下に 7rem 空けるのは、スクロール誘導（下端から 80px）の領分。
        // ここを空けずに等分すると、束の下端が誘導と重なる。
        //
        // 扉（67.5% − 9.6vh/2 ＝ 約62.7svh から）とは、CTA の下端が
        // わずかに触れる。扉は幅 30px ほどの小さな的で、文字のほうが前面に出る
        className="hero-stage flex flex-col items-center justify-center px-6 pb-28 text-center sm:justify-start sm:pb-0 sm:pt-[10vh] md:pt-[13vh]"
      >
        {/* 背景画像（ズーム対象・最背面） */}
