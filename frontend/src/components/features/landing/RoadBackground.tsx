...
export function RoadBackground({ fadeTop, fadeBottom }: RoadBackgroundProps) {
  ...
  useEffect(() => {
    ...
    // 基準はヒーロー終端 + 0.2 画面分（ドア後の余白を短くし早めに始める）。
    // 序盤はセクションのクリップ上端が地平線より下にあるため、実際に道が
    // 見え始めるのはクリップが地平線側に届いてから＝奥から現れる見え方は保たれる。
    ...
    const measure = () => {
      ...
      revealStart = hero ? hero.offsetTop + hero.offsetHeight - ih + ih * 0.2 : 0
      revealLen = ih * 0.75
    }
