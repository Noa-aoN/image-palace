...
> 動作確認に必要なクレジットは、上のお試しコードで足ります（約1クレジットで画像1枚）。
>
...
このアプリでは、**AI の活用そのものも練習の対象としました。**
当時のハーネスエンジニアリングのベストプラクティスをできる限り調査し、
自分なりに組み立てて運用しています（git 管理化は予定）。

組み立てたものの例:

- コミット規約・テスト方針・セキュリティ規則・git ワークフローを、参照される形で明文化する
- 本番と同じ production ターゲットのイメージで RuboCop と RSpec を回す CI（`.github/workflows/`）。
  `REQUIRE_VIPS=1` を置き、依存が欠けたときにテストが黙ってスキップされないようにしている
- 目的別のスキル（実装・レビュー・デプロイ・セキュリティ検査など）を用意し、作業の入口を揃える
- 判断の背景は PR 本文と意思決定ログ（`docs/decisions/`）に残し、後から辿れるようにする
