...
- 通った記録は**端末（devise-token-auth の client）ごと**
- 猶予は**用途で分ける**（記録は同じで、見る窓の広さだけが違う）

| 窓 | 長さ | 何に効くか |
|---|---|---|
| `StrongAuthSession::ADMIN_WINDOW` | **1時間** | 執務室に居られる時間（`/admin` の門） |
| `StrongAuthSession::WINDOW` | **10分** | 危険操作（役割の変更・パスキー削除・二要素の解除・復旧コード再発行） |

執務室に居ること自体は、それだけでは何も壊さない。読んで回るだけの時間で追い出すと、
確かめ直しが作業の邪魔にしかならず「とりあえず通す」癖がつく。
数字を読み、利用者を見て、記録を追う、という一続きの作業は30分では終わらないことが多い。
一方、危険操作は通った直後に効いてしまうので、こちらは短いまま。**入口を広げても中は弱めない。**
- 別の端末で確かめても、こちらは開かない
