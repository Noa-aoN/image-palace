...
[Frontend]                    [Backend]                      [外部サービス]
  ...
  |  { word: "photosynthesis" }  |                                |
  |----------------------------->|                                |
  |                              | NormalizePromptService で正規化  |
  |                              | shared_media キャッシュ確認      |
  |                              |---> キャッシュあり: 即レスポンス  |
  |                              |                                |
  |                              | キャッシュなし:                  |
  ...
  |                              |       | OpenAI Images API      |
  |  ポーリング（定期 GET）        |       |----------------------->|
  |  GET /api/v1/items/:id       |       | 画像 URL + revised_prompt |
  |----------------------------->|       |<-----------------------|
  |                              |       | R2 に保存              |
...
全リトライ消費後に `failed` ステータスへ。ユーザーは `POST /api/v1/items/:id/retry` で手動再生成可能（クレジット消費なし）。
